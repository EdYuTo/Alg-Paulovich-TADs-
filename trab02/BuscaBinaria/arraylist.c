...
int contains_arrayList(ArrayList *arraylist, int chave) {
   if (arraylist != NULL) {
      ...
      for (int i = 0; i < arraylist->numero; i++) {
         if (aux->mchave >= chave) {
            int begin = 0, end = aux->fimvet, middle;
            while (begin <= end) {                       //
               middle = (begin + end)/2;                 //
               if (aux->list[middle]->chave == chave)    //
                  return 1;//contem!                     //
               else if (chave < aux->list[middle]->chave)//-----|> Busca Binaria!
                  end = middle - 1;                      //
               else if (chave > aux->list[middle]->chave)//
                  begin = middle + 1;                    //
            }                                            //
         }
         aux = aux->proximo;
...
int indexOf_arrayList(ArrayList *arraylist, int chave) {
   if (arraylist != NULL) {
      ...
      if (aux != NULL) {
         for (int j = 0; j < i; j++) 
            soma += (10<<j);
         for (i = 0; i < aux->fimvet; i++)//nao devemos usar busca binaria pois precisamos do i, e nao do valor!
