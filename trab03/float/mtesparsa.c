...
int insert(ESPARSA *matrix, int row, int col, float value) {//considera a insercao ja ordenada!
   if (matrix != NULL && row > -1 && col > -1 &&matrix->rows >= row && matrix->cols >= col) {
         ...
         while (auxm1 != m1->head) {//enquanto nao recomeçar...
            ...
            while (auxm1->item->col != -1 || auxm2->item->col != -1) {//se ambos forem -1 para--->estao nos nos iniciais!
               if (auxm1->item->col != -1 && (auxm1->item->col < auxm2->item->col)) {//se nao for no inicial e a col for menor, insere
                  ...
                  float soma = auxm1->item->valor + auxm2->item->valor;
                  if (soma != 0)
...
ESPARSA *mult(ESPARSA *m1, ESPARSA *m2) {
   if (m1 != NULL && m2 != NULL && m1->cols == m2->rows) {
      ...
      float result = 0;//acumular resultados!
  
      if (m3 != NULL) {//segurança!
         while (auxm1 != m1->head) {
            auxm2 = auxm2->right;//desloca porque a parada e justamente ser o 'head', se nao deslocar o laco sera pulado depois da 1 iteracao...
            while (auxm2 != m2->head) {
               auxm1 = auxm1->right;
               auxm2 = auxm2->down;
               while (auxm1->item->col != -1 || auxm2->item->row != -1) {//mesmo while da adicao, mas sem as insercoes! e movendo entre cols e rows!
               ...
               if (result != 0) {//se diferente de 0, insere!
                  insert(m3, auxm1->item->row, auxm2->item->col, result);
                  result = 0;//lembrar de zerar o auxiliar!
               }
               auxm2 = auxm2->right;//no ultimo caso auxm2 = m2->head, por isso no 1 while ha o deslocamento!
            }
            auxm1 = auxm1->down;//desce a linha da multiplicacao!
         }
      }
      return m3;//sucesso!
   }
